...
public class SKMeans {
	...
	public SKMeans(UpdatableSearcher searcher, int paramKappa,
			double paramMaxClusterSize, double paramFacilityCostIncrement) {
		kappa = paramKappa;
	...
	public void processPoint(Centroid p) {
		...
		if (centroids.size() == 0) {
			...
			double probability = p.getWeight() * cn.getWeight()
					/ maxClusterSize;

			// System.out.println("Point [" + p + "]: closest to [" + cn +
			// " Distance: " + cn.getWeight() + " Probability: " + probability);
	...
	private void dismissPoints() {
		...
		while (oldCenters.size() > kappa) {
			...
			// System.out.println("We are trying to dismiss points. Using max cluster size: "
			// + maxClusterSize);
			...
			while (j < oldCenters.size() - 1) {
				...
				// System.out.println("Point [" + p + "]: closest to [" + cn +
				// " Distance: " + cn.getWeight() + " Probability: " +
				// probability);
