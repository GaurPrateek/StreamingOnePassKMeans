...
public class OnePassReducer extends ReduceFunction {
	...
	public void open(Configuration parameters) {
		...
		efinalclusters = Integer.parseInt(parameters.getString(finalclusters,
				"3"));
		emaxIterations = Integer.parseInt(parameters.getString(maxIterations,
				"20"));
		etrimFraction = Float.parseFloat(parameters.getString(trimFraction,
				"0.9f"));
		ekMeansPlusPlusInit = Boolean.parseBoolean(parameters.getString(
				kMeansPlusPlusInit, "false"));
		ecorrectWeights = Boolean.parseBoolean(parameters.getString(
				correctWeights, "false"));
		etestProbability = Float.parseFloat(parameters.getString(
				testProbability, "0.0f"));
		enumRuns = Integer.parseInt(parameters.getString(numRuns, "10"));
		int esearcherTechnique = Integer.parseInt(parameters.getString(
				searcherTechnique, "0"));
		int edistanceTechnique = Integer.parseInt(parameters.getString(
				distanceTechnique, "0"));
		int enumProjections = Integer.parseInt(parameters.getString(
				numProjections, "0"));
		int esearchSize = Integer.parseInt(parameters
				.getString(searchSize, "0"));
		...
		case 0:
			...
			break;
		case 1:
			searcher = new ProjectionSearch(distanceMeasure, enumProjections,
					esearchSize);
			break;
		case 2:
			searcher = new FastProjectionSearch(distanceMeasure,
					enumProjections, esearchSize);
			break;
		case 3:
			searcher = new LocalitySensitiveHashSearch(distanceMeasure,
					esearchSize);
			break;
		}
	}
	...
	public void reduce(Iterator<Record> records, Collector<Record> collector)
		...
		BallKMeans clusterer = new BallKMeans(searcher, efinalclusters,
				emaxIterations, etrimFraction, ekMeansPlusPlusInit,
				ecorrectWeights, etestProbability, enumRuns);
